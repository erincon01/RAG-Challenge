...
Tests helper methods directly and main query methods via mocked psycopg2.
"""
...
def _match_row_no_stadium() -> dict:
    row = _match_row()
    row["stadium_id"] = None
...
def _event_row() -> dict:
    return {
        ...
        "count": 5,
        "json_data": '{"events": []}',
        "summary": "Spain controls possession in midfield",
    }


# ---------------------------------------------------------------------------
# Tests for PostgresMatchRepository._row_to_match
# ---------------------------------------------------------------------------

class TestRowToMatch:
    def setup_method(self):
        self.repo = PostgresMatchRepository.__new__(PostgresMatchRepository)
    ...
    def test_home_team_populated(self):
        ...
        assert match.home_team.team_id == 772
        assert match.home_team.name == "Spain"
        assert match.home_team.manager == "Luis de la Fuente"

    def test_away_team_populated(self):
        match = self.repo._row_to_match(_match_row())
        assert isinstance(match.away_team, Team)
        assert match.away_team.team_id == 768
    ...
    def test_stadium_populated_when_present(self):
        ...
        assert match.stadium.country == "Germany"
    ...
    def test_display_name_format(self):
        match = self.repo._row_to_match(_match_row())
        assert "Spain" in match.display_name
        assert "England" in match.display_name
...
# ---------------------------------------------------------------------------
# Tests for PostgresEventRepository._row_to_event
# ---------------------------------------------------------------------------

class TestRowToEvent:
    def setup_method(self):
        self.repo = PostgresEventRepository.__new__(PostgresEventRepository)
    ...
    def test_event_summary_none_when_absent(self):
        row = _event_row()
        row["summary"] = None
...
# ---------------------------------------------------------------------------
# Tests for PostgresMatchRepository with mocked psycopg2
# ---------------------------------------------------------------------------

def _make_match_repo_with_mock_cursor(rows_fetchone=None, rows_fetchall=None):
    """Build a PostgresMatchRepository where psycopg2.connect is mocked."""
    cursor = MagicMock()
    cursor.fetchone.return_value = rows_fetchone
    cursor.fetchall.return_value = rows_fetchall if rows_fetchall is not None else []
...
class TestPostgresMatchRepositoryQueries:
    ...
    def test_test_connection_returns_false_on_error(self, mock_connect):
        repo = PostgresMatchRepository()
        with patch.object(repo, "get_connection") as mock_gc:
            mock_gc.return_value.__enter__ = MagicMock(side_effect=Exception("conn failed"))
            mock_gc.return_value.__exit__ = MagicMock(return_value=False)
            result = repo.test_connection()
    ...
    @patch("app.repositories.postgres.psycopg2.connect")
    def test_get_all_with_season_filter(self, mock_connect):
        repo = PostgresMatchRepository()
        with patch.object(repo, "get_connection") as mock_gc:
            conn = MagicMock()
            cursor = MagicMock()
            ...
            results = repo.get_all(season_name="2024")
    ...
    def test_get_competitions_valid_rows_returns_entities(self, mock_connect):
        ...
        with patch.object(repo, "get_connection") as mock_gc:
            ...
            cursor.fetchall.return_value = [
                {"competition_id": 55, "competition_country": "Europe", "competition_name": "UEFA Euro"},
                {"competition_id": 11, "competition_country": "Spain", "competition_name": "La Liga"},
            ]
            conn.cursor.return_value.__enter__ = MagicMock(return_value=cursor)
            conn.cursor.return_value.__exit__ = MagicMock(return_value=False)
            mock_gc.return_value.__enter__ = MagicMock(return_value=conn)
...
# ---------------------------------------------------------------------------
# Tests for PostgresEventRepository with mocked psycopg2
# ---------------------------------------------------------------------------

class TestPostgresEventRepositoryQueries:
    @patch("app.repositories.postgres.psycopg2.connect")
    def test_test_connection_returns_true(self, mock_connect):
    ...
    def test_get_events_by_match_no_limit(self, mock_connect):
        ...
        assert len(results) == 2
        assert all(isinstance(e, EventDetail) for e in results)
    ...
    def test_search_by_embedding_invalid_model_raises(self):
        from unittest.mock import MagicMock

        from app.domain.entities import SearchRequest

        req = SearchRequest(match_id=1, query="test", top_n=5)
        # Force an unsupported model by patching the enum lookup
        req_mock = MagicMock()
    ...
    def test_search_by_embedding_invalid_algorithm_raises(self):
...
class TestPostgresMatchRepositoryExceptions:
...
class TestPostgresEventRepositoryExceptions:
