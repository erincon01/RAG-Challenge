...
class TestStatsBombCompetitions:
    def test_returns_200(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
    ...
    def test_response_fields(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_competitions.return_value = COMPETITIONS_RAW[:1]
            data = client.get("/api/v1/statsbomb/competitions").json()
        item = data[0]
    ...
    def test_skips_rows_with_missing_ids(self, client):
        raw = [
        ...
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_competitions.return_value = raw
            data = client.get("/api/v1/statsbomb/competitions").json()
        assert len(data) == 1
        assert data[0]["competition_id"] == 55

    def test_sorted_by_competition_then_season(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_competitions.return_value = COMPETITIONS_RAW
            data = client.get("/api/v1/statsbomb/competitions").json()
    ...
    def test_empty_list_when_no_competitions(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_competitions.return_value = []
            data = client.get("/api/v1/statsbomb/competitions").json()
        assert data == []

    def test_service_error_returns_500(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_competitions.side_effect = Exception("Network error")
            response = client.get("/api/v1/statsbomb/competitions")
...
class TestStatsBombMatches:
    ...
    def test_match_id_in_response(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_matches.return_value = MATCHES_RAW
            data = client.get("/api/v1/statsbomb/matches?competition_id=55&season_id=282").json()
    ...
    def test_skips_rows_without_match_id(self, client):
        raw = [{"match_id": None}, {"match_id": 3943043, "match_date": "2024-07-14"}]
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_matches.return_value = raw
            data = client.get("/api/v1/statsbomb/matches?competition_id=55&season_id=282").json()
        assert len(data) == 1
        assert data[0]["match_id"] == 3943043

    def test_passes_params_to_service(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_matches.return_value = []
            client.get("/api/v1/statsbomb/matches?competition_id=11&season_id=90")
        mock_svc.list_matches.assert_called_once_with(competition_id=11, season_id=90)

    def test_empty_result_for_unknown_competition(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_matches.return_value = []
    ...
    def test_service_error_returns_500(self, client):
        with patch("app.api.v1.statsbomb._service") as mock_svc:
            mock_svc.list_matches.side_effect = Exception("Network failure")
            response = client.get("/api/v1/statsbomb/matches?competition_id=55&season_id=282")
        assert response.status_code == 500
